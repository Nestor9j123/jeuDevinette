...
void jouerPartie(const char *pseudo, Niveau niveau, int borneMax, int tentativesMax) {
    ...
    while (nbTentatives < tentativesMax) {
        if (nbTentatives == tentativesMax - 1) {
            printf("Courage %s, tu peux y arriver !\n", pseudo);
        }
        ...
        if (tentative == nombreADeviner) {
            printf("\nFélicitations %s ! Vous avez deviné le nombre en %d tentatives.\n", pseudo, nbTentatives);
            ...
            do {
               ...
               // Demande si l'utilisateur veut rejouer
                    do {
                        ...
                        fflush(stdin);
                        scanf(" %c", &rejouer);
                        rejouer = tolower(rejouer); // Convertir en minuscule pour simplifier la vérification

                        if (rejouer != 'y' && rejouer != 'n') {
                            printf("Entrée invalide ! Veuillez entrer 'y' pour oui ou 'n' pour non.\n");
                        }
             ...
             // La variable "rejouer" est maintenant utilisée ici
